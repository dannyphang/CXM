...
@Component({
  ...
  templateUrl: './tab-panel-page.component.html',
...
export class TabPanelPageComponent implements OnChanges {
    ...
    this.filterMenuModel = [
      {
        label: this.translateService.instant('BUTTON.ADVANCE_FILTER', {
          numberOfFilter: this.tabFilterList?.length ?? 0
        }),
      ...
      },
      {
        label: this.translateService.instant('BUTTON.EDIT_COLUMN', {
          numberOfColumn: this.tableConfig?.length ?? 0
        }),
  ...
  exportFile(data: any[]) {
    if (this.authService.returnPermissionObj(this.module, 'export')) {
  ...
  filterClick(prop: PropertiesDto) {
    if (!this.tempFilterList?.find((item: any) => item.property === prop)) {
