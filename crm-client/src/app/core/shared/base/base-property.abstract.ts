    ...
    initProfileFormConfig(propertyList: PropertyGroupDto[], module: 'CONT' | 'COMP', contactProfile: ContactDto, companyProfile: CompanyDto, isLeftPanel: boolean, permission: UserPermissionDto[]) {
        ...
        propertyList.forEach(item => {
            ...
            item.propertiesList.forEach(prop => {
                ...
                let control = new FormControl({ value: propProfileValue ? propProfileValue : this.commonService.returnControlTypeEmptyValue(prop), disabled: !this.checkPermission('update', module, permission, this.authService.userC.roleId) || !prop.isEditable });
