...
async function getContactByQuery({ tenantId, query }) {
    return new Promise(async (resolve, reject) => {
        try {
            const { data, error } = await query;
            if (error) {
                console.log("error while getting contact by query", error);
                resolve({ data: null, error }); // resolve with error, don't reject
            } else {
                resolve({ data, error: null });
            }
        } catch (error) {
            console.log("error", error);
            resolve({ data: null, error }); // consistent return shape
        }
    });
}

async function getContactByRPC({ tenantId, rpcFilter }) {
    return new Promise(async (resolve, reject) => {
        try {
            const { data, error } = await supabase.rpc(rpcFilter.function, rpcFilter.params);
            if (error) {
                console.log("error while getting contact by rpc", error);
...
export { getAllContacts, getContactById, getContactAssoList, createContact, updateContact, deleteContact, getContactByQuery, getContactByRPC };
