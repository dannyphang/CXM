...
router.get("/", async (req, res) => {
    try {
        const snapshot = await db.db.collection(activityCollection).where("statusId", "==", 1).where("tenantId", "==", func.body(req).tenantId).orderBy("modifiedDate", "desc").get();
        ...
        if (list.length > 0) {
            for (let i = 0; i < list.length; i++) {
                ...
                const snapshot2 = await db.db.collection(attachmentCollection).where("activityUid", "==", list[i].uid).where("statusId", "==", 1).get();
            ...
            await Promise.all(
                list.map(async (act) => {
                    ...
                    const companyPromises = act.associationCompanyUidList?.map(async (uid) => {
                        const ss = await db.db.collection(companyCollection).doc(uid).get();
                    ...
                    const contactPromises = act.associationContactUidList?.map(async (uid) => {
                        const ss = await db.db.collection(contactCollection).doc(uid).get();
                        return ss.data()?.statusId == 1 ? ss.data() : {};
                    });
                    if (contactPromises) {
                    ...
                    if (act.attachmentUid && act.attachmentUid.length > 0) {
                        const attachmentPromises = act.attachmentUid?.map(async (uid) => {
                            const ss = await db.db.collection(attachmentCollection).doc(uid).get();
                            return ss.data()?.statusId == 1 ? ss.data() : {};
                        });
...
router.get("/activityModule", async (req, res) => {
    try {
        const snapshot = await db.db.collection(moduleCodeCollection).where("moduleType", "==", "ACTIVITY_TYPE").where("statusId", "==", 1).orderBy("moduleId").get();
        const subModuleSnapshot = await db.db.collection(moduleCodeCollection).where("moduleType", "==", "SUB_ACTIVITY_TYPE").where("statusId", "==", 1).orderBy("moduleId").get();
        const actCtrSnapshot = await db.db.collection(moduleCodeCollection).where("moduleType", "==", "ACTIVITY_CONTROL").where("statusId", "==", 1).orderBy("moduleId").get();
        const subActCtrSnapshot = await db.db.collection(moduleCodeCollection).where("moduleType", "==", "SUB_ACTIVITY_CONTROL").where("statusId", "==", 1).get();
...
router.post("/", async (req, res) => {
    try {
        ...
        list.forEach(async (prop) => {
            let newRef = db.db.collection(activityCollection).doc();
            prop.uid = newRef.id;
...
router.post("/upload", async (req, res) => {
    try {
        ...
        list.forEach(async (prop) => {
            let newRef = db.db.collection(attachmentCollection).doc();
            prop.uid = newRef.id;
            prop.createdDate = new Date();
            prop.modifiedDate = new Date();
            prop.statusId = 1;
...
router.delete("/:uid", async (req, res) => {
    try {
        ...
        let actRef = db.db.collection(activityCollection).doc(uid);
