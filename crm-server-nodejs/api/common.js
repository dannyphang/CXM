...
router.get("/" + propertiesCollection, async (req, res) => {
    try {
        let snapshot = await db.default.db.collection(propertiesCollection).where("statusId", "==", 1).orderBy("propertyId").get();
...
router.get("/" + propertiesCollection + "/module", async (req, res) => {
    ...
    try {
        const snapshot = await db.default.db
            ...
            .orderBy("order")
            .get();
        const snapshotModule = await db.default.db.collection(moduleCodeCollection).where("moduleSubCode", "==", moduleCode).where("statusId", "==", 1).get();
        const snapshotPL = await db.default.db.collection(propertiesLookupCollection).where("moduleCode", "==", moduleCode).where("statusId", "==", 1).get();
        ...
        for (let i = 0; i < propertyList.length; i++) {
            ...
            for (let j = 0; j < propertyLookupList.length; j++) {
                if (propertyList[i].propertyId === propertyLookupList[j].propertyId) {
                    propertyLookupList[j].createdDate = convertFirebaseDateFormat(propertyLookupList[j].createdDate);
                    propertyLookupList[i].modifiedDate = convertFirebaseDateFormat(propertyLookupList[j].modifiedDate);
                ...
                if (propertyList[i].propertyType === "CBX_S") {
                    if (propertyLookupList[j].propertyLookupCode === "true") {
                        propertyLookupList[j].createdDate = convertFirebaseDateFormat(propertyLookupList[j].createdDate);
                        propertyLookupList[j].modifiedDate = convertFirebaseDateFormat(propertyLookupList[j].modifiedDate);
                        propertyList[i].propertyLookupList.push(propertyLookupList[j]);
...
router.post("/" + propertiesCollection, async (req, res) => {
    ...
    try {
        ...
        list.forEach(async (prop, index) => {
            ...
            const snapshot = await db.default.db.collection(propertiesCollection).orderBy("propertyId", "desc").limit(1).get();
...
router.post("/" + propertiesLookupCollection, async (req, res) => {
    ...
    try {
        ...
        list.forEach(async (prop, index) => {
            ...
            const snapshot = await db.default.db.collection(propertiesLookupCollection).orderBy("propertyLookupId", "desc").limit(1).get();
...
router.get("/activityModule", async (req, res) => {
    try {
        const snapshot = db.default.db.collection(moduleCodeCollection).where("moduleType", "==", "ACTIVITY_TYPE").where("statusId", "==", 1).orderBy("moduleId").get();
        const actCtrSnapshot = db.default.db.collection(moduleCodeCollection).where("moduleType", "==", "ACTIVITY_CONTROL").where("statusId", "==", 1).orderBy("moduleId").get();
        const subActCtrSnapshot = db.default.db.collection(moduleCodeCollection).where("moduleType", "==", "SUB_ACTIVITY_CONTROL").where("statusId", "==", 1).get();
