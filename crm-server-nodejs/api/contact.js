...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    const contact =
      snapshot.data().statusId == 1 && snapshot.data().tenantId == tenantId ? snapshot.data() : {};
    ...
    if (assoList.length > 0 || assoList2.length > 0) {
      ...
      let p1 = new Promise((resolve, reject) => {
        ...
        assoList.forEach(async (item) => {
          ...
          let cont =
            contactSnapshot.data()?.statusId == 1 && contactSnapshot.data()?.tenantId === tenantId
      ...
      let p2 = new Promise((resolve, reject) => {
        ...
        assoList2.forEach(async (item) => {
          ...
          let cont2 =
            contactSnapshot2.data()?.statusId == 1 && contactSnapshot2?.tenantId === tenantId
...
router.put("/delete", async (req, res) => {
  try {
    func.body(req).data.contactList.forEach(async (contact) => {
